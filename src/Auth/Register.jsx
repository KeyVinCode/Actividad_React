...
const Register = () => {
  ...
  const enviar = (e) => {
    ...
    const existe = datos.some(u => u.Usuario === data.Usuario);
    if (existe) {
      alert('Este usuario ya se encuentra registrado');
      return;
    }
    const nuevalist = [...datos, data];
    localStorage.setItem('usuarios', JSON.stringify(nuevalist));
        ...
        <form onSubmit={enviar} className="min-h-96 px-8 py-6 mt-4 text-left bg-white rounded-xl shadow-lg">
          <div className="flex flex-col justify-center items-center h-full select-none">
            <div className="flex flex-col items-center justify-center gap-2 mb-8">
              <p className="m-0 text-[16px] font-semibold ">Registra tu cuenta</p>
          ...
          <div className="mt-5 justify-center flex flex-col ">
            ...
            <Link className="justify-center flex mt-4" to="/Login">Ya tengo cuenta</Link>
          </div>
        </form>
      </div>
    </div>
